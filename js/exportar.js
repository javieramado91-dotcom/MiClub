...
export function armarHistoria(contenedor, { titulo, equipo, escudo, ranking }) {
    ...
    const filas = ranking.map((j, i) => `
        <div class="ig-fila ${i < 3 ? 'top' + (i + 1) : ''}">
            <span class="ig-puesto">${medallas[i] || (i + 1) + '°'}</span>
    ...
    contenedor.innerHTML = `
        ...
        <div class="ig-firma"><span>Mi Club</span><span class="ig-ja">JA</span></div>
    `;
}
...
export async function descargarComoJPG(contenedor, nombreArchivo) {
    if (typeof html2canvas === 'undefined') {
        mostrarToast("Falta cargar html2canvas. Revisá el <script> en el HTML.", 'error');
        return;
    }
